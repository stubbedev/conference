...
export function trackConstraints(
  ...
  // Audio quality comes from the browser's own processing, asked for
  // explicitly (Safari and Firefox do not all default the same way):
  // echo cancellation, automatic gain and noise suppression, the same
  // trio Google Meet requests. There are no user-facing knobs for it.
  if (kind === 'mic') {
    return {
